/*Вам даны две функции: load_linked и store_conditional (объявления и еще одно объяснение 
принципа работы даны в комментариях к коду). С помощью этих функций реализуйте атомарный 
инкремент и CAS операцию (объявления и описание логики работы даны в комментариях к коду).*/
...
{   
    while (true) {
        int res = load_linked(x);
        if ( store_conditional(x, res + arg) )
            return res;
    }
}
...
{   
    while (true) {
        int res = load_linked(x);
        if ( res != *expected_value ) {                 // достаточно 1 раз !=
            *expected_value = res;
            return false;                               // и на выход
        } else if ( store_conditional(x, new_value) )   // а пока == пытаемся записать новое значение
            return true;                                // если не удалось (значение изменилось)
    }                                                   // пытаемся еще раз, вдруг оно назад вернется
}